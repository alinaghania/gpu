"""
RECIPE 1 — EVENTS SAMPLED
----------------------------------------------
Lit un échantillon d'événements depuis DATASET_MAIN, sur les N derniers mois,
exclut "Aucune_Proposition" (optionnel), impose une profondeur minimale par client,
déduplique (CLIENT_ID, DATE_EVENT), et écrit un dataset prêt pour le SequenceBuilder.
 
parse_dates: on met parse_dates=False côté DSS et on parse nous-mêmes la date.
Gros volumes: mode "chunks" possible.
 
Schema de sortie (inféré automatiquement par write_with_schema) :
...
OUTPUT_DATASET_NAME   = "events_sampled"           #  le dataset de sortie
 
# Colonnes source
CLIENT_ID_COL         = "NUMTECPRS"
TIME_COL              = "DATMAJ"                         # date de fin de mois
PRODUCT_COL           = "SOUSCRIPTION_PRODUIT_1M"
EXTRA_EVENT_COLS      = []                               # ex: ["CANAL", "FAMILLE"]
 
# Échantillonnage
LIMIT_ROWS            = 1_000                          # max lignes à écrire
MONTHS_BACK_TARGET    = 12                               # nb de mois récents à viser
PER_MONTH_CAP         = None                             # None => auto (LIMIT_ROWS // MONTHS_BACK_TARGET)
MIN_CLIENT_MONTHS     = 2                                # profondeur min par client
EXCLUDE_ON_EVENTS     = True                             # exclure "Aucune_Proposition"
RANDOM_SEED           = 42
 
# Lecture en chunks (utile si très gros dataset)
READ_IN_CHUNKS        = True
CHUNKSIZE             = 200
...
def _read_month_sample(
    ...
    if exclude_on_events:
        df = df[df["PRODUCT_CODE"].astype(str).str.strip() != "Aucune_Proposition"]
...
def build_events_stratified(
    ...
    # --- Garanties de non-vide ---
    # F1: si vide avec exclusion, réessaie sans
    ...
    # F2: si encore vide, élargis à + mois (toujours sans exclusion)
    i = len(target_months)
    ...
    if df.empty:
        raise RuntimeError("Aucun événement trouvé après tous les fallbacks.")
    ...
    # F3 : si vide après profondeur, relaxe à 1 mois min (i.e., pas de filtre)
    if df_kept.empty:
        log.warning("Vide après filtre MIN_CLIENT_MONTHS — on relaxe à 1 (pas de filtre).")
        df_kept = tmp.drop(columns=["_m"])
 
    # Équilibrage par PARTITION_MONTH + tronque à LIMIT_ROWS
