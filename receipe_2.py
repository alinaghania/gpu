"""
RECIPE 2 — TRAINING => (CPU-friendly)
----------------------------------
Lit T4REC_EVENTS_SAMPLED, entraîne le modèle hybride T4Rec+Transformer,
et écrit 3 sorties :
  - T4REC_METRICS       : métriques standard + Top-K (validation)
  - T4REC_PREDICTIONS   : top-5 par client (validation)
  - T4REC_MODEL_ARTIFACTS : configs & mapping des classes rares
 
Notes:
- Aucune feature profil ici (RAM friendly).
- On NE re-exclut PAS "Aucune_Proposition" ici (elle a été gérée en Recipe 1).
"""
...
EXTRA_EVENT_COLS = []  # si  ajouté en Recipe 1, liste-les ici aussi , mais j'ai pas ajouté
...
# Fenêtre temporelle souhaitée (on la borne par les mois réellement présents)
WISHED_LOOKBACK = 12
...
def main():
    ...
    log.info("=== RECIPE 2 START (training) ===")
    ds_in = dataiku.Dataset(INPUT_EVENTS_DATASET)
 
    # 1) Lire les événements (tout le dataset ; il est déjà échantillonné )
    ...
    cfg["outputs"]["features_dataset"]        = None  # on n’écrit pas les features ici
    ...
    # 4) (Optionnel) log console rapide
